function funCropImage(filename,append)
    ...
    for n=1:length(files)
        ...
        %resave the image
        if nargin==2 && append
            imwrite(T(ytopedge:ybottomedge,xleftedge:xrightedge,:),[filename(1:end-4) 'cropped' filename(end-3:end)], 'Quality', 100)
        else
            imwrite(T(ytopedge:ybottomedge,xleftedge:xrightedge,:),filename, 'Quality', 100)
        end
    end
