function out = funTexReport(varargin)
    sText = varargin{1};
    sPermission = varargin{2};
    sReportFile = ['../report/DataMiningReport.tex'];
    ...
    switch  length(varargin)
        ...
        case 3
            T = varargin{3};
            table2latex(T,sReportFile,sPermission)
        case 4
            T = varargin{3};
            sReportFile = varargin{4};
            if ~isempty(T) 
                table2latex(T,sReportFile,sPermission)
            end
    end
    
    fid = fopen(sReportFile,sPermission);
    fprintf(fid, newline);
    fprintf(fid, sText);
    fclose(fid);
end

function table2latex(T, filename, sPermission)
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
    ...
    % Writing header
    fileID = fopen(filename, sPermission);
    fprintf(fileID, '{\\scriptsize \\ttfamily \\begin{longtable}{%s}\n \\centering \n ', col_spec);
    fprintf(fileID, '%s \\\\ \n', col_names);
    ...
    try
        for row = 1:size(T,1)
            ...
            if ~isempty(row_names)
                temp = [funSanitizeString(row_names{row}), temp];
            end
            fprintf(fileID, '%s \\\\ \n', strjoin(temp, ' & '));
            clear temp;
        end
